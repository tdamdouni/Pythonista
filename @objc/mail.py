...
def show_mail_controller(subject='', recipients=[], body='', filename=None, mime_type=''):	
	'''show a mail controller, prepopulating with subject, recipients[must be list], body, filename, and attachment mime type string, such as image/gif
	'''
	try:
		MailDelegate = ObjCClass('MailDelegate')
	except: #create new class
		MailDelegate=create_objc_class('MailDelegate',
			superclass=NSObject,   methods=[mailComposeController_didFinishWithResult_error_],  protocols=['MFMailComposeViewController'], debug=True)
		#retain callback
	...
	if isinstance(body,basestring) and body.startswith('<html>'):
		isHTML=True
	else:
		isHTML=False
...
def main():
	...
	def a(sender):
		...
		show_mail_controller('Enlarge you gif! ', ['someone@somewhere.com'],'Try this amazing animated gif!', 'test.gif','image/gif')
	b.action=a
	v.add_subview(b)
