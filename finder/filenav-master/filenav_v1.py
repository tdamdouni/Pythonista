# -*- coding: utf-8 -*-
###############################################################################
# filenav v1 by dgelessus
# http://github.com/dgelessus/filenav/blob/master/filenav_v1.py
###############################################################################
...
APP_DIR = full_path(os.path.join(os.path.dirname(os.__file__), ".."))

def rel_to_docs(path):
    # Return path relative to script library (~/Documents)
    return os.path.relpath(full_path(path), DOCS_DIR)

def rel_to_app(path):
    # Return path relative to app bundle (~/Pythonista.app)
    return os.path.relpath(full_path(path), APP_DIR)
...
if not os.path.exists(os.path.join(DOCS_DIR, "temp")):
    os.mkdir(os.path.join(DOCS_DIR, "temp"))
...
def get_thumbnail(path):
    ...
    try:  # attempt to generate a thumbnail
        ...
        if not err.message == "broken data stream when reading image file":
            return None
        tmp_file = os.path.join(full_path("~"), "Documents/temp/filenav-tmp.png")
...
class StatDataSource(object):
    ...
    def tableview_did_select(self, tableview, section, row):
        ...
        if key == "shellista-cd":
            ...
            destdir = full_path(os.path.join(full_path("~"), "Documents/temp"))
...
if __name__ == "__main__":
    run(sys.argv[1] if len(sys.argv) > 1 else APP_DIR)
