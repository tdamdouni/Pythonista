...
window.CodeMirror = (function() {
  ...
  function compensateForHScroll(display) {
    return getRect(display.scroller, true).left - getRect(display.sizer, true).left;
  }
  ...
  function updateSelection(cm) {
    ...
    var wrapOff = getRect(display.wrapper, true), lineOff = getRect(display.lineDiv, true);
  ...
  function measureLineInner(cm, line) {
    ...
    var outer = getRect(display.lineDiv, false);
    ...
    for (var i = 0, cur; i < measure.length; ++i) if (cur = measure[i]) {
      ...
      if (cur.measureRight) right = getRect(cur.measureRight, false).left;
  ...
  function measureLineWidth(cm, line) {
    ...
    return getRect(end, true).right - getRect(cm.display.lineDiv, true).left;
  }
  ...
  function intoCoordSystem(cm, lineObj, rect, context) {
    ...
    if (context == "page") {
      var lOff = getRect(cm.display.lineSpace, true);
  ...
  function posFromMouse(cm, e, liberal) {
    ...
    var x, y, space = getRect(display.lineSpace, true);
  ...
  function onMouseDown(e) {
    ...
    var editorSize = getRect(display.wrapper, true);
  ...
  function clickInGutter(cm, e) {
    ...
    if (mX >= Math.floor(getRect(display.gutters, true).right)) return false;
    e_preventDefault(e);
    if (!hasHandler(cm, "gutterClick")) return true;

    var lineBox = getRect(display.lineDiv, true);
    if (mY > lineBox.bottom) return true;
    mY -= lineBox.top - display.viewOffset;

    for (var i = 0; i < cm.options.gutters.length; ++i) {
      var g = display.gutters.childNodes[i];
      if (g && getRect(g, true).right >= mX) {
  ...
  function scrollCursorIntoView(cm) {
    ...
    var display = cm.display, box = getRect(display.sizer, true), doScroll = null;
  ...
  CodeMirror.prototype = {
    ...
    coordsChar: function(coords) {
      var off = getRect(this.display.lineSpace, true);
  ...
  function getRect(node, pixel_coord) {
