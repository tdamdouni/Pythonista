...
#                      the associated gist.
#   Create Gist.py8   - create a new gist with the current file
#
#
...
# Credits:
#
# Takes script by davenicholls
# https://gist.github.com/davenicholls/5212628
# and revised the json error  json.loads(r.content) in favor of request.post(...).json
# some minor changed added
#
...
def fork(gist,token):
		...
		r = requests.post(api_url + gist + '/forks',
                       headers=headers)
		return json.loads(r.content)
...
def pull():
	...
	if gist_id is None:
		...
		r = requests.get(api_url + gist_id)
		gist_data = json.loads(r.content)
		try:
...
def commit():
	...
	if m == '': m = None
	gist_id = get_gist_id()
	if os.path.exists(fpath + 'ui'):
		pyuifpath = fpath + 'ui'
		f = open(pyuifpath,'r')
		pyuic = f.read()
		f.close()
		res = commit_or_create(gist_id,{fpath:editor.get_text(),pyuifpath:pyuic },token,m)
	else: 
	...
	try:
			...
			if f == 1:
				...
				try:
					...
					try:
						id = res['id']
						gistAction = 'Fork'
						console.alert('Fork succeeded', 'Forked with id = ' + id, 'Ok')
					except KeyError:
		...
		if gist_id is None:
			set_gist_id(id)
		console.alert('Commit succeeded', 'Comitted with id = ' + id, 'Ok')
...
def download_gist(gist_url):
    ...
    try:
        r = requests.get(api_url + gist_id)
        gist_info = json.loads(r.content)
